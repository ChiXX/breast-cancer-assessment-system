...
- **GET `/assessments`**: 获取评估记录列表。
  - **Query**: `session_id=...` (可选)
- **GET `/assessments/{id}/history`**: 获取关联该评估的会话历史。
- **GET `/assessments/history`**: 获取特定 session 的全量会话历史 (需要 `session_id` query 参数)。

- **POST `/assessments/learn`**: 触发 MCP 知识自进化。
  - **Output**: `{"status": "processing"}`
  - **Logic**: 调用 MCP `/v1/knowledge/learn`。
