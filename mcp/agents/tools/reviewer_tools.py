...
class VerifySkillFormat(BaseTool):
    description = '检查技能资源文件格式。强制要求包含：风险等级、下一步建议、是否建议联系团队、参考依据。缺失参考依据的条目将被移除。'
    parameters = {
        'type': 'object',
        'properties': {
            'skill_name': {
    ...
    def call(self, params: Union[str, dict], **kwargs) -> dict:
        ...
        # 使用 LLM 进行格式审查和清理
        prompt = (
            "你是一个严谨的医疗文档审计员。请检查以下 Markdown 内容中的每一项‘风险分级’或‘知识条目’。\n"
            "要求：每一项必须严格包含以下四个字段：\n"
            "1. **风险等级**\n"
            "2. **下一步建议**\n"
            "3. **是否建议联系团队**\n"
            ...
            "【规则】：\n"
            "- 如果某一项缺失任何字段，尤其是缺失‘参考依据’，请**直接移除**该项。\n"
            "- 保持其余合格条目的内容原封不动。\n"
            "- 只输出修正后的 Markdown 内容，不要有任何解释文字。\n\n"
            f"待审查内容：\n{content}"
        )
...
class ResolveSkillConflicts(BaseTool):
    ...
    def call(self, params: Union[str, dict], **kwargs) -> dict:
        ...
        prompt = (
            "你是一个专业的医疗知识消歧专家。请审查以下技能库中的所有内容，查找是否存在内部矛盾。\n"
            "【检查重点】：\n"
            ...
            "3. 输出格式要求：请使用 JSON 格式返回，键为文件名，值为修正后的内容。格式如：{\"filename.md\": \"content...\"}\n"
