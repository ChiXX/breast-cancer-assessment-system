...
class LearningAgent(BaseMedicalAgent):
    ...
    def __init__(self, llm_cfg: Optional[dict] = None, name: str = 'Learning Agent'):
        ...
        system_prompt = (
            ...
            "4. **【红线约束】原样提炼**：从记忆 JSON 中原样提取 `assessment` 对象并传给工具。**严禁对评估逻辑、风险等级、依据 ID 进行任何修改。**\n"
            ...
            "   - **子资源**：具体的症状评估逻辑存放在 `.md` 文件中（如 `oral-mucositis.md`）。\n"
            "   - **【调用方式】`upsert_skill_resource` 接收 `session_id`、`timestamp`、`resource_name`、`title`，"
            "工具自动从记忆文件读取完整 assessment 并生成 Markdown。你不需要也不应该自己构造 assessment 或文件内容。\n"
            "   - `session_id` 和 `timestamp` 来自记忆列表，例如 session_id=session_7whj6uk5ycu, timestamp=2026-04-28_14-14-15。\n\n"
            "### 【工作流程约束】\n"
